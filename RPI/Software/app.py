...
height = 60 #Terminal window height
width = 128 #Terminal window width
...
def ctrprnt(str, attr):
	global prints
	if len(str) < width:
		pad.addstr(prints,width/2 - len(str)/2,str,attr)
	else:
		pad.addstr(prints,0,str,attr)
	pad.addstr(prints,width,'l')
	pad.refresh(0,0,0,0,height,width)
